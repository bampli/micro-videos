...
describe('Schema Unit Tests', () => {
  ...
  describe('DB_HOST', () => {
    test('invalid cases with abortEarly=true', () => {
      expectValidate(schema, { DB_VENDOR: 'mysql' }).toContain(
        '"DB_HOST" is required',
      );
      expectValidate(schema, { DB_VENDOR: 'mysql', DB_HOST: 1 }).toContain(
        '"DB_HOST" must be a string',
      );
    });

    test('invalid cases with abortEarly=false', () => {
  ...
  describe('DB_PORT', () => {
    ...
    test('valid cases', () => {
      const arrange = [
        { DB_VENDOR: 'sqlite' },
        { DB_VENDOR: 'sqlite', DB_PORT: 8 },
        { DB_VENDOR: 'mysql', DB_PORT: 80 },
        { DB_VENDOR: 'sqlite', DB_PORT: '8' },
        { DB_VENDOR: 'mysql', DB_PORT: '80' },
      ];
  ...
  describe('DB_LOGGING', () => {
    test('invalid cases', () => {
      ...
      expectValidate(schema, { DB_LOGGING: '1' }).toContain(
        '"DB_LOGGING" must be a boolean',
      );
      expectValidate(schema, { DB_LOGGING: 1 }).toContain(
        '"DB_LOGGING" must be a boolean',
      );
      expectValidate(schema, { DB_LOGGING: '0' }).toContain(
        '"DB_LOGGING" must be a boolean',
      );
      expectValidate(schema, { DB_LOGGING: 0 }).toContain(
        '"DB_LOGGING" must be a boolean',
